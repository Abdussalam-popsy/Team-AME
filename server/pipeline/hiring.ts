 ...
 * disqualification decision is never a model judgement. Structured company-size
 * data settles the early-stage requirement; everything else is term matching over
 * profile text plus public research.
 */
  ...
  return input.mustHaves.map((requirement) => {
    if (isEarlyStageRequirement(requirement)) {
      ...
      const shown = early
        ...
        .join(', ');
      return { requirement, met: true, evidence: `worked at ${shown}` };
    }
    ...
    // A term in their own job title is a stronger signal than one term buried in
    // research text, so it settles the requirement on its own.
    const inTitle = terms.some((t) => titles.includes(t));

    // The provider skills filter proves at least one listed skill is on their
    // public skill list, but not which one, so on its own it is only partial.
    const bySkillFilter =
      skillTerms.length > 0 && terms.some((t) => skillTerms.some((s) => s.toLowerCase() === t));

    if (hits.length === 0) {
      return bySkillFilter
        ? {
            requirement,
            met: 'partial',
            evidence: `matched a provider skill filter for ${skillTerms.join('/')}, not corroborated in public text`,
          }
        : { requirement, met: false, evidence: 'no matching term in profile' };
    }
    return {
      requirement,
      met: inTitle || hits.length >= 2 || bySkillFilter ? true : 'partial',
      evidence:
        (inTitle ? `current/past job title contains: ` : `profile mentions: `) +
        [...new Set(hits)].slice(0, 4).join(', ') +
        (bySkillFilter ? `; also matched provider skill filter` : ''),
    };
  });
}
...
export async function runHiringPipeline(
  ...
  for (const p of people) {
    ...
    try {
      ...
      const mustHaves = assessMustHaves(input, plan, p, corpus, skillTerms);
